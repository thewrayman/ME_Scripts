...
--Release Notes
* 2.1.0
    Added Selling Abilities
        top-level usage is the same as buying (example script included at bottom of file)
        specify an item to sell, with the price and quantity
...
function GE.LocateItemInInv(itemId)
    local inv_container = API.Container_Get_s(93,itemId)
    if inv_container ~= nil then
        return inv_container.item_slot
    end
    return false
end
...
function GE.OpenSell(itemId)
    if GE.GEWindowOpen() then
        local itemSlot = GE.LocateItemInInv(itemId)
        if itemSlot then
            API.DoAction_Interface(0xffffffff,itemId,1,107,7,itemSlot,3808)
            API.RandomSleep2(1200,300,600)
            if API.VB_FindPSettinOrder(VBS.SELECTED_ITEM, 0).state == itemId then
                API.logDebug("Successfully selected item for selling")
                return true
            else
                API.logError("Tried to select item for selling but looks like the selection failed")
            end
        end
    else
        API.logError("Opening Sell failed - GE Window not open")
    end
    return false
end
...
function GE.Sell(itemName, itemId, volume, price)
    API.logDebug("Tring to sell " .. itemName)
    local slots = GE.GetAvailableSlots()
    if #slots == 0 then
        API.logError("No GE slots available to carry out a sell offer")
    else
        if GE.OpenSell(itemId) then
            if GE.PlaceOrder(volume, price) then
                GE.ConfirmOrder()
                API.RandomSleep2(1200,300,600)
                GE.CollectToInventory()
                return true
...
-- Will take a list of items to buy, and a list of what to sell. Usage for selling is basically identical to buying.
-- Will go through the buy list first, buy everything it can and then try to sell everything in the sell list
...
-- local itemsToSell = {
--     {NAME = "Bucket", ID = 1925, VOL = 3, PRICE = 1},
-- }

-- local itemsToBuy = {
--     {NAME = "Bucket", ID = 1925, VOL = 3, PRICE = 50000},
-- }
...
--             for _,i in pairs(itemsToSell) do
--                 API.logDebug("Trying to sell ".. i.NAME )
--                 GE.Sell(i.NAME, i.ID, i.VOL, i.PRICE)
--             end            
--             API.Write_LoopyLoop(false)
--         end
--         API.RandomSleep2(600,600,1200)
